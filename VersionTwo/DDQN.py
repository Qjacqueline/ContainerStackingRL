...
class DDQN(BaseAgent):
    ...
    def forward(self, state, action_flag, eval_tag=True):
        ...
        if eval_tag:
            if np.random.rand() <= self.epsilon:  # greedy policy
                x = torch.exp(self.qf.forward(state)) * action_flag.to(self.device)  # q值有正有负但exp后全为正且单调性相同
                action = torch.max(x, 1)[1].item()
            else:  # random policy
                p = np.asarray(action_flag).astype('float64')
                p_sum = np.sum(p, axis=1).reshape(-1, 1)
                p /= p_sum
                action = np.random.choice([i for i in range(len(action_flag[0])) for _ in range(len(action_flag))],
                                          p=p.ravel())
        else:
            x = torch.exp(self.qf(state)) * action_flag.to(self.device)
            action = torch.max(x, 1)[1].item()
        return action
    ...
    def update(self, batch: Dict[str, Any]):
        ...
        q_next = q_next_value.gather(1, torch.max(torch.exp(q_next_value) * action_flag_, 1)[1].unsqueeze(1))
